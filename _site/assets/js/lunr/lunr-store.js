var store = [{
        ...
        "excerpt":"이 글에서는 데이터베이스 정규화에 대해서 알아봅니다. 데이터베이스 정규화의 의미, (정규화를 하지 않았을시 발생할 수 있는) 이상 현상, 정규화의 목적, 정규화의 종류와 방법에 대해서 알아봅니다. 이 글은 위키백과의 데이터베이스 정규화, 제 1 정규형, 제 2 정규형, 제 3 정규형 내용을 참조하였습니다. 1. 데이터베이스 정규화란? 관계형 데이터베이스의 설계에서 중복을 최소화하도록 구조화하는 프로세스를 정규화(Normalization)라고 합니다. 일반적으로 정규화란 크고 제대로 조직되지 않은 테이블들과 관계들을 작고 잘 조직된 테이블과 관계들로 나누는 것을 말합니다. 2. 정규화의 목적 2-1. 데이터베이스의 변경시 이상 현상 제거 정규화를 제대로 하지 않았을 시에는 다음과 같은 문제점이 발생할 수 있습니다. 2-1-1. 갱신 이상 같은 정보가 복수개의 행에서 표현된다면, 갱신 이상(반복된 데이터 중에 일부만 갱신)을 유발합니다. 아래의 예시에서는...","categories": ["database"],
